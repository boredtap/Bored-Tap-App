 ...
 */
const InviteScreen = () => {
  // State for controlling the invite overlay visibility
  const [isOverlayVisible, setOverlayVisible] = useState(false);
  // State for storing the list of invited friends
  const [invites, setInvites] = useState([]);
  // State for the QR code URL generated by the backend
  const [qrCodeUrl, setQrCodeUrl] = useState("");

  // Fetch user profile (including invites) and QR code on mount
  useEffect(() => {
    /**
     * Fetches user profile data (including invited friends) and QR code for sharing.
    ...
    const fetchUserProfileAndQR = async () => {
      ...
      try {
        // Fetch user profile including invited friends
        ...
        setInvites(profileData.invite || []); // Assuming 'invite' field contains friend data

        // Fetch QR code for invite link
  ...
  // Construct invite link using Telegram user ID
  ...
  // Handle copying the invite link to clipboard
  const handleCopy = () => {
