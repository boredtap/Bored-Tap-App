...
const ClanScreen = () => {
  ...
  const [clanPending, setClanPending] = useState(false); // Track if clan is awaiting verification
  ...
  useEffect(() => {
    const fetchTopClansAndStatus = async () => {
      ...
      try {
        ...
        // Check user profile for pending clan status
        ...
        setClanPending(profileData.clan_status === "pending"); // Assuming backend adds this field
      } catch (err) {
  ...
  const handleJoinClan = () => {
    // Placeholder for Join Clan functionality
    console.log("Join Clan clicked");
  };
