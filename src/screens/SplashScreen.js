...
//         console.log("Fetching:", `${BASE_URL}/signin`);
//         const signInResponse = await fetch(`${BASE_URL}/signin`, {
//           method: "POST",
//           headers: { "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json" },
//           body: new URLSearchParams({
...
//         console.log("Signin response headers:", [...signInResponse.headers.entries()]);
//         const authData = await signInResponse.json(); // Read once as JSON
//         console.log("Signin response body:", authData);
...
//           console.log("Signin Success:", { token: authData.access_token });
//         } else {
//           console.log("Signin failed, attempting signup at:", `${BASE_URL}/sign-up`);
//           const signUpBody = {
//             telegram_user_id: telegramUserId,
//             username,
//             image_url: imageUrl,
...
//             body: JSON.stringify(signUpBody),
//           });
//           console.log("Signup response:", { status: signUpResponse.status, body: await signUpResponse.json() });

//           if (!signUpResponse.ok) {
//             const errorData = await signUpResponse.json();
//             if (signUpResponse.status === 400 && errorData.detail?.includes("User already exists")) {
...
//                 }),
//               });
//               console.log("Retry signin response:", { status: retrySignInResponse.status, body: await retrySignInResponse.json() });
//               if (!retrySignInResponse.ok) {
//                 throw new Error(`Retry signin failed: ${JSON.stringify(await retrySignInResponse.json())}`);
//               }
//               authData = await retrySignInResponse.json();
//             } else {
...
//               }),
//             });
//             console.log("Signin after signup response:", { status: signInAfterRegResponse.status, body: await signInAfterRegResponse.json() });
//             if (!signInAfterRegResponse.ok) {
//               throw new Error("Failed to sign in after registration");
//             }
//             authData = await signInAfterRegResponse.json();
...
//         if (userData.photo_url) {
//           console.log("Updating image at:", `${BASE_URL}/bored-tap/user_app`);
//           const imageUpdateResponse = await fetch(
//             `${BASE_URL}/bored-tap/user_app?image_url=${encodeURIComponent(userData.photo_url)}`,
//             {
//               method: "POST",
...
//             }
//           );
//           console.log("Image update response:", { status: imageUpdateResponse.status, body: await imageUpdateResponse.json() });
//           if (!imageUpdateResponse.ok) {
...
//             });
//             console.log("Alt image update response:", { status: altImageUpdateResponse.status, body: await altImageUpdateResponse.json() });
//           }
//         }

//         console.log("Fetching profile from:", `${BASE_URL}/user/profile`);
//         const response = await fetch(`${BASE_URL}/user/profile`, {
//           method: "GET",
//           headers: {
//             Authorization: `Bearer ${authData.access_token}`,
...
//           },
//         });
//         console.log("Profile response:", { status: response.status, body: await response.json() });
//         if (!response.ok) throw new Error("Failed to fetch profile");
//         const data = await response.json();
...
//       const token = authData.access_token;
//       console.log("Fetching extra boosters from:", `${BASE_URL}/user/boost/extra_boosters`);
//       const extraBoostersResponse = await fetch(`${BASE_URL}/user/boost/extra_boosters`, {
//         method: "GET",
//         headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
//       });
//       console.log("Extra boosters response:", { status: extraBoostersResponse.status, body: await extraBoostersResponse.json() });
...
//     initializeAuth();
//   }, [navigate, resetAll, setLastActiveTime, setExtraBoosters, setAutoTapActive, setTotalTaps, setElectricBoost]);
