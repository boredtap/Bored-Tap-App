...
const BoostScreen = () => {
      ...
      setDailyBoosters((prev) => {
        ...
        if (boosterType === "tapperBoost") {
          updated.tapperBoost = {
            ...
            resetTime: booster.usesLeft === 1 ? now + DAILY_RESET_INTERVAL : booster.resetTime, // Set only when uses reach 0
          };
  ...
  const renderTimer = (boosterType) => {
    ...
    if (boosterType === "tapperBoost" && booster.isActive) {
      ...
      const hours = Math.floor(resetIn / 3600).toString().padStart(2, "0");
      const minutes = Math.floor((resetIn % 3600) / 60).toString().padStart(2, "0");
      const seconds = Math.floor(resetIn % 60).toString().padStart(2, "0");
      return `${booster.usesLeft}/3 (${hours}:${minutes}:${seconds})`;
    }
