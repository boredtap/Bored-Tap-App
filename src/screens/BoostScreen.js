...
const BoostScreen = () => {
  ...
  const handleClaimDailyBooster = (boosterType) => {
    const booster = dailyBoosters[boosterType];
    if (booster.usesLeft > 0) {
      const endTime = Date.now() + 3600000; // 1-hour cooldown per use
      setDailyBoosters((prev) => ({
        ...prev,
        [boosterType]: {
          usesLeft: booster.usesLeft - 1,
      ...
      if (boosterType === "tapperBoost") {
        // Activate Tapper Boost: Double tap income for 20 seconds
        window.dispatchEvent(new CustomEvent("activateTapperBoost"));
        console.log("Tapper Boost activated: x2 tap income for 20 seconds");
      } else if (boosterType === "fullEnergy") {
        // Activate Full Energy: Fill energy to total energy cap
        window.dispatchEvent(new CustomEvent("activateFullEnergy"));
        console.log("Full Energy activated: Energy filled to total energy cap");
      }
    }
    handleOverlayClose(); // Slide overlay out
  };
  ...
  return (
    <div className="boost-screen">
              ...
              onClick={() =>
                setActiveOverlay({
                  ...
                  description:
                    booster.type === "tapperBoost"
                      ? "Multiply your tap income by x2 for 20 seconds."
                      : "Fill your energy to 100% instantly. 3 uses per day.",
                  value: "Free",
                  icon: booster.icon,
                })
              }
      ...
      {activeOverlay && (
        <div className={`overlay-container ${activeOverlay ? "slide-in" : "slide-out"}`}>
          <div className="overlay-card">
            <div className="overlay-header">
              <h2 className="overlay-title">{activeOverlay.title}</h2>
              <button className="overlay-close" onClick={handleOverlayClose}>✖</button>
            </div>
            <hr className="division-line" />
            <img src={activeOverlay.icon} alt={activeOverlay.title} className="overlay-icon" />
            <p className="overlay-description">{activeOverlay.description}</p>
            <div className="overlay-value-container">
              <img src={`${process.env.PUBLIC_URL}/logo.png`} alt="Coin Icon" className="value-icon-large" />
              <p className="overlay-value">{activeOverlay.value}</p>
            </div>
            <button
              className="overlay-cta"
              onClick={() => handleClaimDailyBooster(activeOverlay.type)}
            >
              Claim
            </button>
          </div>
